...
using PlutoUI, Random, HypertextLiteral, StyledStrings
...
But now let's consider that this game happens m times. Now your friend can retaliate in the future, and maybe cooperation is viable in the long run. Axelrod's tournament explores this scenario. 

In this post, you will play this game against six of the most famous strategies proposed, each during around 10 rounds. 
"""
...
<div style="display: flex; gap: 12px;">
    ...
    <div>
        $(@bind next CounterButton("Next opponent / Finish"))
    </div>
</div>
""")
...
begin
    ...
    if reset > game.last_reset
        ...
        game.strategy_order = shuffle(1:length(strategies))
        game.current_strategy = 1
        game.turn = 0

        game.turns_per_match = 10 + rand(-3:3)
        ...
        println(styled"{underline:Turn 1}")
		println()
    ...
    # MATCH STATE 
    # =========================
    match_active = game.turn < game.turns_per_match


    # =========================
    # PLAY TURN 
    # =========================
    if your_choice !== nothing &&
       game.current_strategy <= length(strategies) &&
       match_active
        ...
        println(styled"{underline:Turn $(game.turn)}")
    ...
    # MATCH END MESSAGE 
    # =========================
    if game.turn == game.turns_per_match && next <= game.last_next &&
   game.current_strategy <= length(strategies)
    ...
    # NEXT BUTTON 
    # =========================
    if next > game.last_next && !match_active

	    game.last_next = next
	
	    # If it's not the last strategy
    ...
    if game.current_strategy > length(strategies) 
		...
		# =========================
	    # opponent vs oppoennt
	    # =========================
        n = length(strategies)
        strategy_scores = zeros(Int, n)

        game.turns_per_match = 10
...
manifest_format = "2.0"
project_hash = "eafe68afab286067919d1e1eb6d0b2d11b91b6eb"
